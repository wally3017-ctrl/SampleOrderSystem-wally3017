...
```
RESERVED → (승인) → 가용 재고 충분: CONFIRMED → RELEASED
                 → 가용 재고 부족: PRODUCING → CONFIRMED → RELEASED
         → (거절) → REJECTED
```

> **가용 재고** = 물리 재고 - 현재 CONFIRMED 상태 주문 수량 합계
> CONFIRMED 상태 주문은 출고 예정 수량으로 간주하여 가용 재고에서 차감한다.
...
- 승인 시 **가용 재고** 기준으로 자동 분기
  - 가용 재고 충분(가용 재고 ≥ 주문 수량) → **CONFIRMED** (재고 차감 없음, 출하 예약 상태)
  - 가용 재고 부족 → 생산라인 자동 등록 + **PRODUCING**
- 거절 시 즉시 REJECTED

> 재고 차감은 출고(RELEASED) 시점에 이루어진다. 승인 시에는 차감하지 않는다.

### 4.4 생산라인

- 단일 라인, FIFO 스케줄링
- 실 생산량: `ceil(부족분 / (수율 × 0.9))`
  - 수율 안전계수 0.9는 불량률 여유분을 반영한다.
- 총 생산 시간: `평균 생산시간(min) × 실 생산량`
- 생산 완료 조건: `등록 시각 + 총 생산 시간(분) ≤ 현재 시각`
- **자동 완료 처리**: 화면 전환 시마다 완료 조건을 확인하여 PRODUCING → CONFIRMED 자동 전환
  - 생산 완료 시 실 생산량만큼 물리 재고 증가
...
- 재고량: 시료별 물리 재고 수량 + 재고 상태 표기
  - 여유: 물리 재고 ≥ RESERVED 주문 수량
  - 부족: 물리 재고 < RESERVED 주문 수량
  - 고갈: 물리 재고 = 0
- 메인 메뉴 진입 시마다 전체 요약 정보 자동 갱신
...
| 시점 | 물리 재고 변화 |
|------|--------------|
| 주문 접수 (RESERVED) | 변화 없음 |
| 주문 승인 → CONFIRMED | 변화 없음 (가용 재고에서만 차감) |
| 주문 승인 → PRODUCING | 변화 없음 |
| 생산 완료 (→ CONFIRMED) | **+실 생산량** |
| 출고 처리 (→ RELEASED) | **-주문 수량** |
...
| 빌드 | MS Build 옵션 변경 금지 (v145 toolset, C++20) |
...
| 2 | 시료 주문 | 주문 접수 및 승인/거절 |
| 4 | 모니터링 | 상태별 주문 수 및 재고 현황 |
| 5 | 생산라인 조회 | 생산 중 작업 및 대기 큐 |
